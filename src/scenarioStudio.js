...
import { RULE_EVENTS, CONDITION_TYPES, ACTION_TYPES, createRule, validateRule, validateScenarioMapRuleIdentity } from "./scenarioRules.js";
...
function createUniqueRule(rules=[]) {
  const ids = new Set((rules||[]).map(rule => String(rule?.id ?? "")));
  let index = 1;
  while (ids.has(`rule-${index}`)) index++;
  const rule = createRule(index);
  rule.name = `Rule ${(rules||[]).length + 1}`;
  return rule;
}
...
function renderInspector(){
  ...
  bindInspector(obj);renderRuleList(ui.objectRules,obj.rules||[],rules=>{remember();obj.rules=rules;renderPanels();},{type:"object",objectId:obj.id});
}
...
function ruleCard(rule,index,onChange,scope={type:"global"}){
  ...
  const runtimeId=scope.type==="object"?`${scope.objectId||"object"}:${rule.id||"(missing)"}`:(rule.id||"(missing)");
  card.innerHTML=`<div class="rule-head"><h4>${escapeHtml(rule.name||`Rule ${index+1}`)}</h4><button class="rule-remove danger">×</button></div><div class="mini-grid"><label>Rule name<input data-r="name" value="${escapeAttr(rule.name||"")}"></label><label>Rule ID<input data-r="id" value="${escapeAttr(rule.id||"")}" spellcheck="false"></label></div><p class="subtle">Runtime ID: <code>${escapeHtml(runtimeId)}</code></p><div class="mini-grid"><label>When<select data-r="event">${eventOptions}</select></label><label>Condition<select data-r="condition">${conditionOptions}</select></label></div><label>Condition value<input data-r="conditionValue" value="${escapeAttr(condition.value??condition.owner??condition.tag??"")}"></label><div class="mini-grid"><label>Then<select data-r="action">${actionOptions}</select></label><label>Value<input data-r="actionValue" value="${escapeAttr(action.value??action.text??action.objectId??"")}"></label></div><div class="mini-grid"><label class="check"><input data-r="once" type="checkbox" ${rule.once?"checked":""}> Once</label><label>Cooldown<input data-r="cooldown" type="number" min="0" value="${rule.cooldown||0}"></label></div>`;
  ...
  const commit=()=>{
    rule.name=card.querySelector('[data-r="name"]').value;rule.id=card.querySelector('[data-r="id"]').value;rule.event={...(rule.event||{}),type:card.querySelector('[data-r="event"]').value};
...
function renderGlobalRules(){renderRuleList(ui.globalRules,state.map.globalRules||[],rules=>{remember();state.map.globalRules=rules;renderPanels();},{type:"global"});}
...
function ruleIdentityMessage(error){const id=String(error?.id??"").trim();if(error?.code==="RULE_ID_DUPLICATE")return`Runtime rule ID collision: ${id||"(blank)"}. Rename one Rule ID or its owning object ID.`;return`Runtime rule ID missing at rule ${Number(error?.index||0)+1}. Give every rule a non-empty Rule ID.`;}
function renderValidation(){state.map.variables=parseVariables();const result=validateMapDefinition(state.map);const identity=validateScenarioMapRuleIdentity(state.map);const blocks=[];blocks.push(`<div class="${result.valid?"ok":"err"}">${result.valid?"✓ Schema valid":"✕ Schema errors"}</div>`);result.errors.forEach(x=>blocks.push(`<div class="err">${escapeHtml(x)}</div>`));result.warnings.forEach(x=>blocks.push(`<div class="warn">${escapeHtml(x)}</div>`));let ruleErrors=0;for(const rule of [...state.map.globalRules,...Object.values(COLLECTIONS).flatMap(key=>(state.map[key]||[]).flatMap(o=>o.rules||[]))])if(!validateRule(rule).valid)ruleErrors++;if(ruleErrors)blocks.push(`<div class="err">${ruleErrors} invalid rule(s)</div>`);else blocks.push(`<div class="ok">✓ Rules structurally valid</div>`);if(identity.valid)blocks.push(`<div class="ok">✓ Runtime rule IDs unique</div>`);else{blocks.push(`<div class="err">✕ Runtime rule identity held</div>`);identity.errors.forEach(error=>blocks.push(`<div class="err">${escapeHtml(ruleIdentityMessage(error))}</div>`));}ui.validation.innerHTML=blocks.join("");return identity;}
function admitRuleIdentity(action){state.map.variables=parseVariables();const identity=renderValidation();if(identity.valid)return true;const first=identity.errors[0];const detail=first?.code==="RULE_ID_DUPLICATE"?`duplicate runtime ID ${String(first.id??"")}`:"missing runtime rule ID";flash(`HELD · ${detail} · fix before ${action}`);ui.validation.scrollIntoView?.({block:"nearest"});return false;}
